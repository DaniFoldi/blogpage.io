...
function articlelink_open(tokens, idx) {
  var tagName = tokens[idx].content.toLowerCase();
  return '<a href="/articles/' + tagName + '" class="article">';
}

function articlelink_close() {
  return '</a>';
}

function articlelink_text(tokens, idx) {
...
module.exports = function mention_plugin(md, options) {
    ...
    for (j = 0, l = blockTokens.length; j < l; j++) {
      ...
      for (i = tokens.length - 1; i >= 0; i--) {
        ...
        for (m = 0; m < matches.length; m++) {
          ...
          token = new Token('articlelink_open', '', 1);
          token.content = tagName;
          token.level = level++;
          nodes.push(token);

          token = new Token('articlelink_text', '', 0);
          token.content = escapeHtml(tagName);
          token.level = level;
          nodes.push(token);

          token = new Token('articlelink_close', '', -1);
          token.level = --level;
          nodes.push(token);
  ...
  md.core.ruler.after('inline', 'articlelink', articlelink);
  md.renderer.rules.articlelink_open = articlelink_open;
  md.renderer.rules.articlelink_text = articlelink_text;
  md.renderer.rules.articlelink_close = articlelink_close;
};
